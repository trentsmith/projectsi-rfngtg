...
var net;
  ...
  for(var i =0;i<this.tickers.length;i++)
  {
    ...
    url1 = "https://api.worldtradingdata.com/api/v1/history?symbol="+this.tickers[i]+"&sort=newest&api_token=PRt07UGBzUrZPTUROLww1CBKxFeS0OcWL7qB3oaaJKxeFJ1mWuhz7LI08qBF";
    ...
    this.http.get(url1).subscribe(
      (data:any[])=>
      {
        ...
        for(var j =0;j<this.tickerssectors[i].length;j++)
{
  console.log(this.tickerssectors[i][j]);
     url2 = "https://api.worldtradingdata.com/api/v1/history?symbol="+this.tickerssectors[i][j]+"&sort=newest&api_token=PRt07UGBzUrZPTUROLww1CBKxFeS0OcWL7qB3oaaJKxeFJ1mWuhz7LI08qBF";
       ...
       for( var j = 100;j > 2;j--)
        {
          ...
          var volume1 = this.temp["history"][this.currentDate]["volume"];
          //note to calculate percentage
          if( open < close&&count>0)
          {
            var a = [this.temp["history"][this.currentDate].open, this.temp["history"]
            [this.currentDate]["volume"]];
            this.getDate(j+1);
